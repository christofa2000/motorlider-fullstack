...
export default async function ProductDetailPage(props: PageProps) {
  const { id } = props.params instanceof Promise ? await props.params : props.params;
  ...
  const DetailHeader = () => (
    <header className="card card--flat p-3 md:p-4 mb-4 sticky top-0 z-30">
      <div className="container flex items-center gap-3">
        ...
        <h1 className="text-lg md:text-xl font-bold ml-1">Detalle del producto</h1>
      </div>
    </header>
  );
  ...
  return (
    <>
      ...
      <div className="grid gap-8 md:grid-cols-2">
        {/* GalerÃ­a */}
        <section aria-label="GalerÃ­a de imÃƒÂ¡genes del producto" className="space-y-3">
          ...
          <div className="grid grid-cols-4 gap-2">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="aspect-video rounded-md bg-[var(--color-neutral-200)]" />
            ))}
          </div>
        </section>

        {/* Lado derecho: precio, CTA, disponibilidad */}
        <section aria-label="InformaciÃ³n del producto" className="flex flex-col gap-4">
          ...
          {/* Ficha tÃƒÂ©cnica / envÃƒÂ­o (placeholder) */}
          <section aria-label="Ficha tÃƒÂ©cnica" className="space-y-2">
            <h2 className="text-lg font-semibold title">Ficha tÃƒÂ©cnica</h2>
            <ul className="text-sm  list-disc pl-5">
              <li>CategorÃ­a: {product.category?.name ?? "-"}</li>
      ...
      <section className="mt-12">
        <h2 className="text-lg font-semibold title">Productos relacionados</h2>
